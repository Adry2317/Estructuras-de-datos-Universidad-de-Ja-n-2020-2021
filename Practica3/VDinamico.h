...
private:    
    ...
    //Definimos el vector.
    T *vect;
    //Tiempo que tarda en realizar las busqueda total de los elementos.
    clock_t tiemposBusqueda = 0;
...
public:
    ...
    VDinamico(unsigned int tamL):tamLogico(tamL){
        ...
        if(tamL >= 0){
           ...
           throw (invalid_argument ("El tamaño lógico dado en el constructor no puede se menor que 0"));
        }
    }
                                                  ...
                                                  
    {
        ...
        if((tamLogico - inicio)>= num || (inicio >= 0 && inicio < tamLogico)){
            ...
            throw out_of_range ("No es posible la copia parcial de este vector");
        }
    ...
    VDinamico<T>& operator=( const VDinamico &v){
        if(this != &v){
            ...
            throw invalid_argument("Ambos vectores son los mismos");
        }
        return *this;
    }
    ...
    T& operator[](unsigned int pos){
        if(pos < 0 || pos > tamLogico)
            throw out_of_range("La posición a la que intenta acceder está fuera del rango");
    ...
    void insertar (const T& dato, unsigned int pos = UINT_MAX){
        ...
        if (pos != UINT_MAX){
            if(pos > tamLogico || pos < 0){
                throw invalid_argument ("Posición del vector no válida");
            }
    ...
    void borrar(unsigned int pos = UINT_MAX){
        ...
        if( pos != UINT_MAX){
             ...
             if( pos < 0 || pos > tamLogico){
                throw invalid_argument("Posición del vecto no válida");
             }
    ...
    int busquedaBin(T& dato){
        ...
        while( inf <= sup){
            ...
            if(vect[mitad] == dato){
               this->tiemposBusqueda += ((clock() - t_init));
                return mitad;
    ...
    /**
     * Función que devuelve el tiempo de busqueda de la estructura
     * @return: tiempoBusqueda.
     */
    clock_t getTiempoBusqueda(){
        return this->tiemposBusqueda;
    }
